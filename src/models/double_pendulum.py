...
class DoublePendulumModel:
    ...
    def get_derivative(state, t, m1, m2, l1, l2, g=9.81):
        ...
        # Společná část jmenovatele podle PDF: [m1 + m2 * sin^2(theta1 - theta2)]
        ...
        # d(omega_1)/dt = úhlové zrychlení 1 (přesně podle vzorce z PDF)
        dydx[1] = (
        ...
        # d(omega_2)/dt = úhlové zrychlení 2 (přesně podle vzorce z PDF)
        dydx[3] = (
    ...
    def simulate(state_0, t_max=30, dt=0.02, m1=1.0, m2=1.0, l1=1.0, l2=1.0):
        ...
        # Řešení soustavy diferenciálních rovnic pomocí odeint [cite: 130]
        sol = odeint(DoublePendulumModel.get_derivative, state_0, t, args=(m1, m2, l1, l2))

        # Extrakce úhlů z výsledku [cite: 139]
        theta1 = sol[:, 0]
        theta2 = sol[:, 2]

        # Převod polárních souřadnic na kartézské (x, y) [cite: 140]
        x1 = l1 * np.sin(theta1)
        y1 = -l1 * np.cos(theta1)
